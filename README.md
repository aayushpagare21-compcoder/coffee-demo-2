...
**Currently pasted: the v3 snippet, on STAGING** — a preconnect to the staging
assets CDN (`#opti-snippet-preconnect`,
`https://assets.staging.optimeleon.com`), an inline bootstrap
(`#opti-snippet-inline`, guard `__opti_af_v = 3`) that injects
`<style id="__opti_af">` at runtime (800 ms failsafe, a 10 s MutationObserver,
and the reveal function exposed as `window.__opti_af_r`), followed by two
async tags (`#opti-snippet-async-1/2`) pointing at
`https://edge-staging.optimeleon.com`, site key `pFAjbs8VjA2v`. The preconnect
and the async tags all carry the `itemProp` anti-hoisting opt-out — React 19
treats both `<link rel="preconnect">` and `<script async src>` as hoistable
resources. The three script tags carry `nowprocket` and `data-no-minify="1"`
verbatim from the shipped snippet — inert WordPress-plugin opt-out markers.
